...
const (
	...
	// defaultAWSRegion 是未配置 AWS_REGION 时使用的默认区域。
...
func LoadConfig(ctx context.Context) error {
	start := time.Now()
	utilities.LogStart(configLoaderComponent, "LoadConfig")

	if !IsRunInAWS() {
		if err := loadFromDotEnv(); err != nil {
			utilities.LogWarn(configLoaderComponent, "LoadConfig",
	...
	if err := validateRequiredKeys(); err != nil {
		utilities.LogError(configLoaderComponent, "LoadConfig", err, time.Since(start), "step=validate")
		return err
	}

	utilities.LogSuccess(configLoaderComponent, "LoadConfig", time.Since(start),
...
func loadAWSConfig(ctx context.Context, start time.Time) {
	if getS3Bucket() != "" && getS3Key() != "" {
		if err := loadFromS3Env(ctx); err != nil {
			utilities.LogWarn(configLoaderComponent, "loadAWSConfig",
	...
	utilities.LogProgress(configLoaderComponent, "loadAWSConfig",
...
func loadFromDotEnv() error {
	start := time.Now()
	utilities.LogStart(configLoaderComponent, "loadFromDotEnv")

	if err := godotenv.Load(); err != nil {
		if os.IsNotExist(err) {
			utilities.LogWarn(configLoaderComponent, "loadFromDotEnv",
				"未找到 .env 文件，使用系统环境变量",
	...
	utilities.LogSuccess(configLoaderComponent, "loadFromDotEnv", time.Since(start), "file=.env")
	return nil
}
...
func loadFromS3Env(ctx context.Context) error {
	start := time.Now()
	utilities.LogStart(configLoaderComponent, "loadFromS3Env")

	bucket := getS3Bucket()
	key := getS3Key()
	region := AWSRegion(defaultAWSRegion)

	utilities.LogProgress(configLoaderComponent, "loadFromS3Env",
		"正在从 S3 下载 .env 文件",
	...
	utilities.LogSuccess(configLoaderComponent, "loadFromS3Env", time.Since(start),
		fmt.Sprintf("bucket=%s", bucket),
		fmt.Sprintf("key=%s", key),
...
func loadFromSecretsManager(ctx context.Context) error {
	start := time.Now()
	utilities.LogStart(configLoaderComponent, "loadFromSecretsManager")
	...
	utilities.LogProgress(configLoaderComponent, "loadFromSecretsManager",
		"正在从 Secrets Manager 加载配置",
		fmt.Sprintf("secret=%s", utilities.Mask(secretName)),
		fmt.Sprintf("region=%s", region),
	)
	...
	if err != nil {
		return fmt.Errorf("Secrets Manager GetSecretValue 失败 (secret=%s): %w",
			utilities.Mask(secretName), err)
	}

	if output.SecretString == nil {
		return fmt.Errorf("Secrets Manager 返回空密钥值 (secret=%s)",
			utilities.Mask(secretName))
	}
	...
	utilities.LogSuccess(configLoaderComponent, "loadFromSecretsManager", time.Since(start),
		fmt.Sprintf("secret=%s", utilities.Mask(secretName)),
	)
	return nil
}
...
func injectSecretToEnv(secretJSON string) error {
	...
	utilities.LogProgress(configLoaderComponent, "injectSecretToEnv",
		"注入完成",
		fmt.Sprintf("injected=%d", injected),
		fmt.Sprintf("skipped=%d", skipped),
	)
